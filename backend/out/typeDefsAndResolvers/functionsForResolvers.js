...
export const newSale = async (sale) => {
    ...
    try {
        ...
        const user = await User.findOneAndUpdate({ _id: userId }, { $addToSet: { forSale: newSale } }, { new: true });
        if (user != null) {
            await user.save();
            return user;
        }
...
export const deleteUserFunction = async (username, password) => {
    try {
        //connecting to db
        await mongoose.connect('mongodb+srv://MrRobots25:KFaQvEBfLrC76xNE@cluster.tt1mykg.mongodb.net/');
...
// find sales
export const FindSales = async (product) => {
    try {
        //connecting to db
        await mongoose.connect('mongodb+srv://MrRobots25:KFaQvEBfLrC76xNE@cluster.tt1mykg.mongodb.net/');
...
export const newChatRoomRequestFunction = async (seller, buyer, saleId, status) => {
    try {
        //connecting to db
        await mongoose.connect('mongodb+srv://MrRobots25:KFaQvEBfLrC76xNE@cluster.tt1mykg.mongodb.net/');
        ...
        if (sale != null) {
            ...
            if (ForSale != null && newChatRoomRequest != null) {
                ...
                const fundSaleAndUpdate = await ForSale.findOneAndUpdate({ _id: saleId }, { $addToSet: { chatRoomRequests: newChatRoomRequest } }, { new: true });
...
export const changeChatRoomReqStatus = async (args) => {
    // variables
...
export const getChatReqsForSale = async (saleId) => {
    try {
        //connecting to db
        await mongoose.connect('mongodb+srv://MrRobots25:KFaQvEBfLrC76xNE@cluster.tt1mykg.mongodb.net/');
        console.log('connected');
...
export const getBuyersChatReqs = async (args) => {
    const { buyerId } = args;
    try {
        //connecting to db
        await mongoose.connect('mongodb+srv://MrRobots25:KFaQvEBfLrC76xNE@cluster.tt1mykg.mongodb.net/');
